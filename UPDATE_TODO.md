...
### 🚀 다음 배포 및 스토어 업데이트 작업
- [ ] **1. 앱 버전 업데이트 (v1.1.3)** — `build.gradle` (versionCode +1) 및 `package.json` 버전 수정
- [ ] **2. 웹 빌드 및 네이티브 동기화** — `npm run build` 및 `npx cap sync android` 실행
- [ ] **3. AAB 빌드 및 업로드** — 최신 코드 빌드 후 구글 플레이 콘솔 비공개 테스트 트랙 업로드
- [ ] **4. 출시 노트 작성** — "UI/UX 개선: 전 화면 뒤로가기 버튼 위치 및 상단 노치 영역 최적화" 
- [ ] **5. 구글 스토어 업데이트 심사 제출**

---

## ⏳ v1.1.3 업데이트 항목 (다음 빌드에 포함)

| # | 항목 | 상태 |
|---|------|------|
| 1 | 모든 서브 화면 상단 헤더 패딩 최적화 (Notch/Safe-area 대응 완료, 뒤로가기 잘림 방지) | ✅ 개발 완료 |
| 2 | 커뮤니티 화면(게시글 피드, 글쓰기, 게시글 상세) 뒤로가기 버튼 노치 간섭 해결 | ✅ 개발 완료 |
| 3 | AI 수의사 화면 헤더 패딩 44px로 확대 및 커뮤니티/히스토리/프로필 상단 뒤로가기 버튼 통일 | ✅ 개발 완료 |
