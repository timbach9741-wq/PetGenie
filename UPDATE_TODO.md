# 🚀 Pet Genie 업데이트 관리 (2026-09-02 현재)

> **현재 버전**: v1.1.10 (versionCode 30)
> **출시 상태**: 🕒 구글 Play 스토어 프로덕션 심사 제출 완료 (v1.1.10, 9월 2일) — 심사 대기 중
> **추가 사항**: ✅ v1.1.9에서 추가한 실제 AdMob 배너가 화면 전환 시 `BannerExecutor` NullPointerException을 유발하던 크래시를 v1.1.10에서 수정
...
### 🚀 Google Play Console 진행 현황 (v1.1.10 기준)

> 🕒 **상태: 프로덕션 트랙 심사 제출 완료, 심사 대기 중 (2026-09-02 제출 건)**
> 배너 광고 크래시 수정을 포함한 AAB 빌드(`PetGenie_v1.1.10_20260902_201527.aab`)를 9월 2일에 업로드하였고, 프로덕션 트랙에 검토 제출까지 완료했습니다. 구글 심사 통과 대기 중.

- [x] **1. 앱 번들 업로드 준비** — 최신 AAB 파일 업로드 완료 (2026-09-02)
- [x] **2. 스토어 정보 최신화** — 릴리스 노트 작성 완료
- [x] **3. 프로덕션 트랙 제출** — 구글 플레이 심사 제출 완료 (2026-09-02)
- [ ] **4. 구글 프로덕션 심사 통과** — 대기 중

---

## ✅ v1.1.10 완료 항목 (2026-09-02)

| #   | 항목                                                                      | 파일                              |
| --- | -------------------------------------------------------------------------- | --------------------------------- |
| 1   | Crashlytics `BannerExecutor` NullPointerException 크래시 수정 (화면 전환 시 `AdMob.showBanner`/`removeBanner`가 겹쳐 호출되던 race condition을 promise 큐로 직렬화) | `src/lib/adMobBanner.ts`(신규), `src/components/common/AdBanner.tsx` |
| 2   | versionCode 30 / versionName 1.1.10으로 상향                              | `android/app/build.gradle`, `package.json` |
...
| #   | 항목                                                                      | 파일                              |
| --- | -------------------------------------------------------------------------- | --------------------------------- |
| 1   | 실제 AdMob 배너 광고 연동 (기존엔 가짜 placeholder만 존재)              | `AdBanner.tsx`                    |
| 2   | 성장 단계 무료 개방 정책: 모든 프리미엄 기능 2026-12-31까지 전원 무료 개방 (`FREE_FOR_ALL` 플래그) | `App.tsx`                         |
...
1. **현재 출시 상태**: ✅ 구글 Play 스토어 1.1.8 프로덕션 심사 통과 및 출시 완료
